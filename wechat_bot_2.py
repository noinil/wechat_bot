        ...
        if self.robot_switch:
            for i in stop_cmd:
                if i == msg_data:
                    ...
                    self.send_msg_by_uid(u'[Bot.v2] ' + u'机器人已关闭！', msg['to_user_id'])
            if msg_data == away_cmd and self.away_status == False:
                self.away_status = True
                self.send_msg_by_uid(u'[Bot.v2] ' + u' Master away from keyboard！', msg['to_user_id'])
            elif msg_data == back_cmd and self.away_status == True:
                self.away_status = False
                self.send_msg_by_uid(u'[Bot.v2] ' + u' Master welcome back！', msg['to_user_id'])
            elif msg_data == ai_on_cmd and self.ai_status == False:
                self.ai_status = True
                self.send_msg_by_uid(u'[Bot.v2] ' + u' AI turned on！', msg['to_user_id'])
            elif msg_data == ai_off_cmd and self.ai_status == True:
                self.ai_status = False
                self.send_msg_by_uid(u'[Bot.v2] ' + u' AI turned off！', msg['to_user_id'])
        else:
            for i in start_cmd:
                if i == msg_data:
                    self.robot_switch = True
                    self.send_msg_by_uid(u'[Bot.v2] ' + u'机器人已开启！', msg['to_user_id'])
    ...
    def handle_msg_all(self, msg):
        ...
        if msg['msg_type_id'] == 1 and msg['content']['type'] == 0:  # reply to self
            ...
            if self.away_status == True:
                self.send_msg_by_uid(u'[Bot.v2] ' + u' 主人不在, 请稍后联系! ', msg['user']['id'])
                reply = '[Bot.v2] '
                response = subprocess.Popen(['fortune', '-a'], stdout=subprocess.PIPE).communicate()[0]
                reply += response
                ...
                if is_at_me:
                    ...
                    if msg['content']['type'] == 0:  # text message
                        if self.ai_status == True:
                            ...
                            response2 = u'(你好, 我是第二代机器人.  请直接输入 "好无聊" 或 "来首诗歌" 获得安慰!) 想要 AI, 请联系主人开启 AI 模式!'
                            reply += response2
                    else:
                    ...
                    if msg['content']['type'] == 0:  # text message
                        ...
                        if u'好无聊' in user_input:
                            ...
                            reply = '[Bot.v2] '
                            response = '喝碗鸡汤吧, 地球人! (请直接输入 "好无聊" 或 "来首诗歌" 获得安慰!)'
                            response2 = subprocess.Popen(['fortune', '-e', 'chinese'], stdout=subprocess.PIPE).communicate()[0]
